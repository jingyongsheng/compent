...
public class Runnable implements java.lang.Runnable {
	...
	public static final Logger log = LoggerFactory.getLogger(Runnable.class);
	...
	public void refresh(String filePath, Long beforeTime) throws Exception {
		...
		List<File> refreshs = getRefreshFile(new File(filePath), beforeTime);
		if (refreshs.size() > 0) {
	...
	public List<File> getRefreshFile(File dir, Long beforeTime) {
		...
		for (File file : files) {
			if (file.isDirectory()) {
				refreshs.addAll(getRefreshFile(file, beforeTime));
			} else if (file.isFile()) {
				if (check(file, beforeTime)) {
					refreshs.add(file);
				}
			} else {
